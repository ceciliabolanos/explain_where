...
for i, xin in enumerate(dataset_test):
    ...
    pred = ud_model(xin)
    if xin['emotion'] != 1 and (pred[0].item() > 0):  # The word little is not in the audio but the model finds it
...
kws_dataset.to_csv('/home/ec2-user/explain_where/datasets/kws/kws_fp_dataset.csv', index=False)

# Here the explanation should be "Why does the model say that the word is in the audio?"