...
for i, xin in enumerate(dataset_test):
    ...
    pred = ud_model(xin)
    if xin['emotion'] == 1 and (pred[0].item() < 0): # The word little is in the audio but the model doesn't find it
...
kws_dataset.to_csv('/home/ec2-user/explain_where/datasets/kws/kws_fn_dataset.csv', index=False)

# Here the explanation should be "Why does the model say that the word is not in the audio?"