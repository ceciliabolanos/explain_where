...
def get_with_name_audioset(method: str, name, n_sample, base_path: str, dataset, intersection, id):
    ...
    for i in tqdm(range(len(selected_files))):
        ...
        file_path = f'{base_path}/explanations_{dataset}/{filename}/ast/ft_{id}_{name}_samples{n_sample}.json'
    ...
    pred_df.to_csv(os.path.join(output_dir, f'auc_relaxed_{method}_{name}_samples{n_sample}_{intersection}.tsv'), sep='\t', index=False)


def get_with_name(method: str, name, n_sample, base_path: str, dataset, intersection):
    results = []
    
    for root, _, files in tqdm(os.walk(os.path.join(base_path, f'explanations_{dataset}'))):
        pattern = re.compile(rf'ft_.*_{name}_samples{n_sample}\.json$')
