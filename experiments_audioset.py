    ...
    for window_size in window_sizes: 
        for mask_percentage in mask_percentages:
            ...
            generate_explanation(
                ...
                id_to_explain=0,
                config=mask_config,
                path='/home/ec2-user/results1/explanations_audioset1'
            )
for config in mask_configs:
    # id = int(selected_files.loc[i]['event_label'])
    id = 0
    name=list(config.keys())[0]
    # second_path = f"/home/ec2-user/results1/explanations_audioset/{filename}/ast/ft1_{id}_{name}.json"
