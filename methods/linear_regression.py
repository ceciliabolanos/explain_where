...
class LimeBase:
    ...
    def explain_instance_with_data(self, neighborhood_data, neighborhood_labels, 
        ...
        try:
            p_values = stats.coef_pval(model_regressor, 
                                     neighborhood_data[:, features],
                                     neighborhood_labels)
        except np.linalg.LinAlgError:
            p_values = np.ones_like(model_regressor.coef_) * 0.05
        ...
        return (model_regressor.intercept_,
                [model_regressor.coef_, p_values],
                prediction_score,
                local_pred)
...
class LimeAudioExplainer:
    ...
    def explain_instance(self, label_to_explain=None, model_regressor=None, random_seed=42):
        ...
        explanation = AudioExplanation(np.array(data['snrs']), y)
