...
def predict_with_yamnet(filename, label):
    wav_data, sample_rate = sf.read(f'/home/ec2-user/Datasets/audioset_eval_wav/{filename}.wav')
    if sample_rate != 16000:
        wav_data = resample(wav_data, int(len(wav_data) * 16000 / sample_rate))
    if len(wav_data.shape) > 1 and wav_data.shape[1] == 2:
    ...
    return scores.numpy().tolist()[label]

# Summing scores for each segment per model
# df['score_yamnet'] = df.apply(lambda row: predict_with_yamnet(row['base_segment_id'], row['father_id_yamnet']), axis=1)
